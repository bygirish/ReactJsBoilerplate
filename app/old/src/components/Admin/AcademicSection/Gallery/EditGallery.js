...
updateAlbum = () => {
  ...
  formData.append('id_album',this.state.selectedalbumId);
  ...
  new Service().apiCall('Gallery/updateAlbum', formData,
  {
    headers: {
      ...
      this.setState({
        basicNotify: (
          ...
          <div className="text-center p-5">
            ...
            <h4 className="font-weight-bold mt-4">Album Updated Successfully!</h4>
          </div>
        </Dialog>
        ),
      });
...
setTags = (tag) => {
  ...
  renderImage = () => {
    let img = null;
    let data = this.state.images[this.state.img_index];
    if(this.state.images.length > 0){
     img = typeof data == "string" ? data : URL.createObjectURL(data);
    }
    return img;
  }
  ...
  getAlbumImages = () => {
    ...
    new Service().apiCall('Gallery/getAlbumDataWithId',postData).then(response => {
      ...
      this.setState({ albumName:response.data[0].name,albumDescription:response.data[0].description, captions:captions, tags:tags, images:files, selectedStandards:response.data[0].selectedstandards, selectedDate:moment(response.data[0].album_date).format('YYYY-MM-DD'),selectedAlbumName:response.data[0],selectedFile:response.data[0].cover,imagePreviewUrl:response.data[0].cover?Config.path+"writable/uploads/gallery/covers/"+response.data[0].cover : this.state.imagePreviewUrl }); 
      }
    }).catch(error => {
      this.showError(error.response.data)
    });
  ...
  removeFile(i) {
    ...
    this.setState({
      ...
      captions: captions.filter((file, index) => index !== i),
      tags: tags.filter((file, index) => index !== i),
    });
  }
    ...
    <Fragment>
          ...
          <Card className="card-box mb-4 pt-3">
            <Grid container spacing={2}>
              ...
              <Grid container style={{height:'250px'}}>
                ...
                <h6 className="mt-2 text-center">EDIT ALBUM COVER</h6> 
      ...
      <Grid item xs={6} sm={4} md={2} lg={2}>
          <Card className="card-transparent mb-4">
            <div className="card-img-wrapper">
              ...
              <img src={typeof data == "string" ? data : URL.createObjectURL(data)} className="card-img-top rounded" alt="..." />
            </div>
      ...
      {this.state.images.length > 0 &&   <Grid container  justify="center">
        ...
        <Grid item xs={12} sm={12} lg={4}>
          <Button className="m-2 w-100 py-2 font-18" variant="contained" color="secondary" onClick={()=>this.updateAlbum()}>
          Update Album
          </Button>
        </Grid>
        <Grid item xs={12} sm={12} lg={4}></Grid>
                ...
                <Grid item  style={{width:'360px'}}>
                  ...
                  <Grid container spacing={4} className="mt-2">
                                    <Grid item   xs={12} sm={12} md={12} className="text-center">
                                     {this.state.images.length > (this.state.img_index+1) && <Button className="mr-2" size="small" variant="outlined" color="primary" onClick={()=>this.setState({img_index : this.state.img_index + 1})}>
                                      Save & Continue
                                     </Button> }
                                      <Button size="small" variant="outlined" className="successBtnOutline mr-2" onClick={()=>this.updateAlbum()}>
                                      Save & Exit
                                      </Button> 
                  </Grid>
                  </Grid>
