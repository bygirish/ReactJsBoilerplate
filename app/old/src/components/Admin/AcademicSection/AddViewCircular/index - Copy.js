        ...
        if (response.status==200 && response.data!='') {
          var lCirculars = [];
            ...
            response.data.forEach(element => {
              ...
              lCircular.title               = element.title;
              lCircular.standrad            = element.standrad;
              lCircular.start_date          = element.start_date;
              lCircular.end_date            = element.end_date;
              lCircular.details             = element.details;
              lCircular.path                = element.path;
              lCirculars.push(lCircular);
          });
               this.setState({ circularsData:lCirculars});
        }
      }).catch(error => {
        console.log(error)
        //this.showError(error.response.data)
    ...
    <Fragment>
     ...
     <Grid container spacing={2}>
      ...
      <Grid item xs={12} lg={6} className="py-1">
        ...
        {(this.state.actionType == "view" || this.state.actionType == "assessment") && <Grid container spacing={4}  justify={this.props.data.type!= "1" ? "none" : "center"}>
        {this.props.data.type!= "1" && <Grid item xs={12} md={8} lg={3}>
