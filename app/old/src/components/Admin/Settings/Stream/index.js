...
class ClassMaster extends React.Component {
  ...
  handleClassMasterDelete = (id, status) => {
    ...
    new Service().apiCall('ClassMasters/deleteStream', postData,
      {
        headers: {
          ...
          basicNotify: (
            <Dialog open={true}>
              <div className="text-center p-5">
                <h4 className="font-weight-bold">{status == 1 ? "Stream Deactivated" : "Stream Activated"}</h4>
              </div>
            </Dialog>
          ),
        });
  ...
  getStreamData(id_board, id_academicyear) {
    const postData = {
      id_organization: this.props.data.selectedOrganizationId,
      id_institute: this.props.data.selectedInstitutionId,
      id_board: this.state.selectedBoard,
