...
class ClassMaster extends React.Component {
  ...
  handleClassMasterDelete = (id, status) => {
    ...
    new Service().apiCall('ClassMasters/deleteBoard', postData,
      {
        headers: {
          ...
          basicNotify: (
            <Dialog open={true}>
              <div className="text-center p-5">
                <h4 className="font-weight-bold">{status == 1 ? "Board Deactivated" : "Board Activated"}</h4>
              </div>
            </Dialog>
          ),
        });
  ...
  handleClassStream = e => {
    ...
    console.log(postData)
    new Service().apiCall('ClassMasters/insertBoard', postData,
      {
        headers: {
          'content-type': 'multipart/form-data'
        }
       ...
       if(response.data == 201){
        ...
        this.setState({
          basicNotify: (
            <Dialog open={true}>
              <div className="text-center p-5">
                <h4 className="font-weight-bold">Board Added</h4>
              </div>
            </Dialog>
          ),
        });
  ...
  getBoard(id_board, id_academicyear) {
    const postData = {
      id_organization: this.props.data.selectedOrganizationId,
      id_institute: this.props.data.selectedInstitutionId,
      id_board: this.state.selectedBoard,
    ...
    new Service().apiCall('ClassMasters/getBoard', postData).then(response => {
      if (response.status == 200 && response.data != '') {
        console.log(response)
        if (response.data) {
          let data = [];
                              ...
                              <Grid item xs={12} sm={10} md={5} >
                                <FormControl fullWidth>
                                  <TextField
                                    ...
                                    required
                                    onChange={(event) =>
                                      this.handleClassMasterDataStream(idx, "name", event.target.value)
                                    }
