...
class RoomFeeManagement extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      alert: null,
      roomholders: [{ bill_date: '', bill_no: '', odometer_reading: '', fuel_station: '', driver_name: '', discount: '0', adjustment: '0'
      , total: 0,TDS:0,payment_date:0,transaction_id:0,paying_amount:0,paying_account_no:0}],
      getFuelDetailsWithVehicleId: [{ bill_date: '', bill_no: 0, odometer_reading: 0, fuel_station: '', driver_name: '', discount: '0', adjustment: '0'
      , total: 0,paid_status:'' }],
      total:'0',
      subTotal:'0',
      subTotalAmt:'0',
      balance:0,
      paidTillDate:0,
      expenseDetails:[{particular:'',quantity:'0',fuel_cost:'0'}],
      loading: true,
      selectedBoard: '',
      basicNotify: false,
  ...
  handleDeactive = (id, status) => {
    ...
    new Service().apiCall('TransportationMasters/deleteFuelDetails', postData).then(response => {
  ...
  insertBlock = () => {
    ...
    let odometer_reading=  datac[0]['odometer_reading'];
    ...
    formData.append('odometer_reading', odometer_reading);
    ...
    // new Service().apiCall('TransportationMasters/insertFuelData', postData).then(response => {
      new Service().apiCall('TransportationMasters/insertFuelDataFile', formData,
      {
        headers: {
  ...
  insertBlockUnpaid = () => {
    ...
    let id_fuel=  PaymentDetails[0]['id_fuel'];
    ...
    formData.append('paying_amount', paying_amount);
    formData.append('paying_account_no', paying_account_no);
    formData.append('id_fuel', id_fuel);
    formData.append('id', id_fuel);
    
    console.log(...formData);
    // new Service().apiCall('TransportationMasters/insertFuelData', postData).then(response => {
      new Service().apiCall('TransportationMasters/FuelUnpaidTransactionDetails', formData,
      {
        headers: {
          'content-type': 'multipart/form-data'
        }
      ...
      if (response.status == 200 && response.data != '') {
        ...
        this.getDetails();
        this.getBalance();
        this.HandlePaidStatus("paidStatus",id_fuel);
        setTimeout(() => {
          this.setState({ basicNotify: false, roomholders: [{ bill_date: '', bill_no: '', odometer_reading: '', fuel_station: '', driver_name: '', discount: 0, adjustment: 0
          , total: 0,TDS:0,balance:0 }],
          total:'0',
  ...
  updateHeading = (id, index) => {
    ...
    new Service().apiCall('TransportationMasters/updateFuelDetails', postData).then(response => {
      if (response.status == 200 && response.data != '') {
        this.setState({
          basicNotify: (
            <Dialog open={true}>
  ...
  addBlock = () => {
    ...
    let object = {particular:'',quantity:'0',fuel_cost:'0'};
    data.push(object);
    this.setState({ data });
    this.calculateTotal();
  }
  ...
  calculateTotal = () => {
    ...
    if(this.state.expenseDetails.length > 0){
       sum = this.state.expenseDetails.map(o => o.fuel_cost * o.quantity).reduce((a, c) => { return Math.ceil(a) + Math.ceil(c) });
       let datac = this.state.roomholders;
       ...
       subTotal = this.state.expenseDetails.map(o =>  o.quantity).reduce((a, c) => { return Math.ceil(a) + Math.ceil(c) });
  ...
  HandlePaidStatus=(paidStatus,id)=>{
    ...
    if(id){
      ...
      new Service().apiCall('TransportationMasters/getPaymentDetails', postData).then(response => {
        console.log(response);
        if (response.status == 200 && response.data != ' ') {
          const visitorList = response.data.map((data) => {
            return { ...data, checked: false, editable: false};
  ...
  handleDate = (index,name,val) => {
    this.setState({ datePay: val })
  };
  handleDateUnpaid = (index,name,val) => {
    this.setState({ datePayUnpaid: val })
  };
                              ...
                              <Grid item xs={12} sm={10} md={4}>
                                <FormControl fullWidth>
                                  ...
                                  <TextField
                                    ...
                                    onChange={(event) => this.handleChangeData(index, "odometer_reading", event.target.value)}
                                    value={element.odometer_reading}
                                    id="document-type"
                                    label="Odometer reading while refueling"
                                    type="search"
                                    variant="outlined" />
                                </FormControl>
                              </Grid>
                              ...
                              <Grid item xs={12} sm={10} md={2}>
                                <FormControl fullWidth>
                                  <TextField
                                    ...
                                    onChange={(event) => this.handleChangeDataED(index, "quantity", event.target.value)}
                              ...
                              <Grid item xs={12} sm={10} md={3}>
                                <FormControl fullWidth>
                                  <TextField
                                    ...
                                    onChange={(event) => this.handleChangeDataED(index, "fuel_cost", event.target.value)}
                                    value={element.fuel_cost}
                                    id="document-type"
                                    label="Fual Cost Rs."
                                    type="search"
                                    variant="outlined" />
                         ...
                         {this.state.paidStatus==true && 
                          <Grid container spacing={4} align='center'>
                            ...
                            <Grid item lg={6} md={6} sm={12} xs={12}>
                              ...
                              <Grid item xs={12} md={7} lg={7}>
                                                                <FormControl fullWidth className="pb-0">
                                                                    <RadioGroup row aria-label="position" name="PaymentMode" value={this.state.PaymentMode} onChange={(event) => this.setState({PaymentMode:event.target.value})} defaultValue="cash">
                                                                    <FormControlLabel value="cash" control={
                                                                        <Radio name="PaymentMode" color="primary"
                                                                            />} label="Cash"
...
//           selectedPaidStatusId:rowInfo.original['id'],
          
//         });
//         this.HandlePaidStatus()
