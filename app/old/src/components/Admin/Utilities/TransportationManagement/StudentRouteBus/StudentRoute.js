...
class StaffToVehicle extends React.Component {
    ...
    handleDeactive = (id, status) => {
        let headingStatus = "Student Route Activated!";
        if (status == 1) {
            headingStatus = "Student Route Deactivated!";
        }
        ...
        new Service().apiCall('TransportationRouteMasters/UpdateStudentRouteStatusMapping', postData).then(response => {
    ...
    getRoleMaster = (id, type) => {
        ...
        const id_route= split[split.length - 1];
        const postData = {
          id_route:id_route,
      ...
      insertStafftoRole = () => {
        ...
        let id_stop = this.state.role_name;
        const split = window.location.href.split("/");
        
        const id_route= split[split.length - 1];
        // const id_trip = split[split.length - 1];
        // this.setState({ id_trip:id_trip });
        ...
        const postData = {
            ...
            id_board:this.props.data.selectedBoardId,   
            id_academicyear:this.props.data.selectedAcademicId,  
            stopDetails:this.state.staffArr,
            token:"abc",
            id_route:id_route,
        ...
        console.log(postData);
        //return false;
        new Service().apiCall('TransportationRouteMasters/insertStudentRouteNew',postData).then(response => {
            console.log(response);
        if (response.status==200 && response.data!='') {
            this.setState({
            basicNotify: (
    ...
    getBlocksData = (id, type) => {
        ...
        // const id_trip = split[split.length - 1];
        // this.setState({ id_trip:id_trip });
        // console.log('id_route '+id_route+'trip_no '+id_trip);
        const postData = {
            id_organization: this.state.selectedOrganizationId,
            id_institute: this.state.selectedInstitutionId,
            token: "abc",
        ...
        new Service().apiCall('TransportationRouteMasters/getStudentRouteDetails', postData).then(response => {
            console.log(response)
            if (response.status == 200 && response.data != '') {
                const data = response.data.map((data) => {
                    return { ...data, checked: false, editable: false, gross_rent: 0 };
    ...
    searchStaff = (event,values) => {
        ...
        if(values){
            const listItems = values.map((myList, key) =>  
                dataV.push(myList.UID)         
            ); 
        }
        data[index][name] = dataV;
        this.setState({ data });
        // console.log(data)
                        ...
                        <Toolbar className="w-100">
                            <Grid container>
                                <Grid item xs={12} lg={12} className="d-flex">
                                    <IconButton edge="start" color="inherit" onClick={() => this.props.history.push("/admin/student-route-bus")} aria-label="close">
                                        <CloseIcon />
                                    </IconButton>
                                            ...
                                            <Grid container spacing={4}>
                                                <Grid item xs={12} md={12} lg={3}>
                                                    <FormControl component="fieldset">
                                                        <FormLabel component="legend" className="pt-2">Select Stop Name:</FormLabel>
                                                    </FormControl>
                                                </Grid>
                                                <Grid item xs={12} md={12} lg={5}>
                                                    <FormControl fullWidth>
                                                        <TextField                        
                                                            ...
                                                            value={original.id_stop}
                                                            onChange={(event)=>this.handleChangeData(key,"id_stop", event.target.value)
                                                            }>
                                                            {this.state.RoleMaster.map(option => (
                                                            <MenuItem key={option.stop_name} value={option.id} id={option.stop_name}>
                                                                {option.stop_name}
                                                            </MenuItem>
                                                            ))}
                                                        </TextField>
