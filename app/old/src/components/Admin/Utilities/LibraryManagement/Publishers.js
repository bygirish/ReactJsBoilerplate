...
class Publishers extends React.Component {
  ...
  updatePublisher = (id, index) => {
    let data = this.state.publishers;
    ...
    new Service().apiCall('Libraries/updatePublisher',postData).then(response => {
      if (response.status==200 && response.data!='') {
        this.setState({
          basicNotify: (
            <Dialog open={true}>
            <div className="text-center p-5">
            <h4 className="font-weight-bold">Publisher Updated</h4>
            </div>
            </Dialog>
          ),
        });
...
rowEdit = (estatus,index) => {
  ...
  let lheadings = this.state.publishers;
  if(estatus == true){
 ...
 // console.log(lheadings);
  this.setState({ publishers:lheadings});
}
  renderEditable = (cellInfo) => {
   
    const cellValue = this.state.publishers[cellInfo.index][cellInfo.column.id];
  ...
  handleDeactive = (id,status) => {
    let switchStatus = "";
     if(status == true){
        switchStatus = "Publisher Deactivated";
     }
     else{
        switchStatus = "Publisher Activated";
     }
     const postData = {
      id_organization:this.state.selectedOrganizationId,
      id_institute:this.state.selectedInstitutionId,
  ...
  createPublisher = () => {
    ...
    new Service().apiCall('Libraries/insertPublisher',postData).then(response => {
      if (response.status==200 && response.data!='') {
        this.setState({
          basicNotify: (
            <Dialog open={true}>
    <div className="text-center p-5">
      <h4 className="font-weight-bold">Publisher  Created</h4>
    </div>
  </Dialog>
          ),
        });
  ...
  return (
    ...
    <Grid container spacing={5} justify="center">
        ...
        <FormControl fullWidth>
              ...
              variant="outlined"
              className="inputTag"
              label={"Add Publisher and press 'Enter'"}
        ...
        {this.state.tags.length > 0 &&  <Button className="successBtnOutline" variant="outlined" onClick={()=>this.createPublisher()}>
              Submit
            </Button>}
        </Grid>
        </Grid>}
...
data={
this.state.publishers.map((original,key) => {
return ({
  slno: key+1,
    ...
    <div className="grouplist-actions">
      ...
      >
         { original.editable ?  <Button disabled={original.status == 0}
                            ...
                            simple
                            onClick={()=> {this.setState({selectedHeading:original.publisher}); this.updatePublisher(original.id,key);}}
                            color="secondary"
                            className="edit"
                          >
                            <CheckCircleOutline  />
                          ...
                          simple
                          onClick={()=> {this.setState({selectedHeading:original.publisher}); this.rowEdit(original.editable,key);}}
                          color="secondary"
                          className="edit"
                        >
                          <Edit  />
