...
class Languages extends React.Component {
  ...
  updateLanguage = (id, index) => {
    ...
    new Service().apiCall('Libraries/updateLanguage',postData).then(response => {
      if (response.status==200 && response.data!='') {
        this.setState({
          basicNotify: (
            <Dialog open={true}>
            <div className="text-center p-5">
            <h4 className="font-weight-bold">Language Updated</h4>
            </div>
            </Dialog>
          ),
        });
...
rowEdit = (estatus,index) => {
  ...
  let lheadings = this.state.languages;
  if(estatus == true){
 ...
 // console.log(lheadings);
  this.setState({ languages:lheadings});
}
  renderEditable = (cellInfo) => {
   
    const cellValue = this.state.languages[cellInfo.index][cellInfo.column.id];
  ...
  handleDeactive = (id,status) => {
    let switchStatus = "";
     if(status == true){
        switchStatus = "Language Deactivated";
     }
     else{
        switchStatus = "Language Activated";
     }
     const postData = {
      id_organization:this.state.selectedOrganizationId,
      id_institute:this.state.selectedInstitutionId,
    ...
    new Service().apiCall('Libraries/deletePublish',postData).then(response => {
      if (response.status==200 && response.data!='') {
        this.setState({
          basicNotify: (
            <Dialog open={true}>
  ...
  createLanguage = () => {
    ...
    new Service().apiCall('Libraries/insertLanguage',postData).then(response => {
      if (response.status==200 && response.data!='') {
        this.setState({
          basicNotify: (
            <Dialog open={true}>
    <div className="text-center p-5">
      <h4 className="font-weight-bold">Language  Inserted</h4>
    </div>
  </Dialog>
          ),
        });
  ...
  return (
    ...
    <Grid container spacing={5} justify="center">
        ...
        <FormControl fullWidth>
              ...
              variant="outlined"
              className="inputTag"
              label={"Add Language and press 'Enter'"}
        ...
        {this.state.tags.length > 0 &&  <Button className="successBtnOutline" variant="outlined" onClick={()=>this.createLanguage()}>
              Submit
            </Button>}
        </Grid>
        </Grid>}
...
data={
this.state.languages.map((original,key) => {
return ({
  slno: key+1,
    ...
    <div className="grouplist-actions">
      ...
      >
         { original.editable ?  <Button disabled={original.status == 0}
                            ...
                            simple
                            onClick={()=> {this.setState({selectedHeading:original.languages}); this.updateLanguage(original.id,key);}}
                            color="secondary"
                            className="edit"
                          >
                            <CheckCircleOutline  />
                          ...
                          simple
                          onClick={()=> {this.setState({selectedHeading:original.language}); this.rowEdit(original.editable,key);}}
                          color="secondary"
                          className="edit"
                        >
                          <Edit  />
...
export default connect(mapStateToProps, mapDispatchToPros)(withRouter(Languages));
