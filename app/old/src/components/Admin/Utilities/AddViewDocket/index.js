  ...
  return (
    <Fragment>
      ...
      <AppBar className="app-header" color="secondary" position="fixed">
          <Toolbar className="w-100">
            <Grid container>
               ...
               <IconButton edge="start" color="inherit" onClick={()=>this.props.history.push("/admin/document-management")} aria-label="close">
              <CloseIcon />
            </IconButton>
      ...
      <Grid container spacing={4} justify="center">  
        ...
        {this.state.studentInfo!='' &&  AuthHelper('Document Management','can_create') &&  <div><Grid container spacing={4} justify="center">
...
variant="outlined" />
      ...
      <Grid item xs={12} sm={10} md={7}></Grid>
      <Grid item xs={12} sm={10} md={4} className="text-right pr-2">
        ...
        <Grid item xs={12} sm={12} md={1} className="margin-auto text-center">
             <Button className="successBtnOutline" variant="outlined" onClick={()=>this.insertDocument()}>Submit</Button>
        </Grid>
        </Grid>
     </Card></Grid></Grid></div>}

       {this.state.studentDocuments.length > 0 && <Grid container spacing={4} justify="center">
        <Grid item xs={12} md={10} lg={8}>
...
data={
  this.state.studentDocuments.map((original,key) => {
    return ({
      slno: key+1,
      id:original.id,
      id_document:original.id_document,
      name: original.name+" "+ original.middle_name+" "+original.last_name,
      standard:original.standard,
      parent:original.father_name,
      contact:original.contact_number,
      status:original.document_status,
      submitted_on:original.submitted_on,
      actions: (
        <div>
            <Tooltip
            id="tooltip-top"
            ...
            //modified in bug fixing
            onClick={()=>this.renderStatusPanel(original.document_status,original.id, original)}
            >
            <ViewIcon />
            </Button>
            </Tooltip>
