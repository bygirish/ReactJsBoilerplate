...
class StudentDashboard extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      activeStep:0,
      joiningStandard:'',
      steps:['Student Profiling', 'Sibling Profiling', 'Parents Profiling', 'Fee Configuration', 'Attachments'],
      requiredState:"",
      basicNotify:false,
      searchStudent:false,
      admission_number:'',
  ...
  handleStudentSearch = (val) => {
    this.setState({'sibling_id':val.UID,});
  }
  ...
  handleChangeInstitute = (idx, value) => {
    ...
    lsiblingata.map((siblings,id)=>{ 
      if(idx==id){
        siblings.sibling_institute=value;
      } 
    });
  ...
  handleBoard= (id,name,status) => {
      this.setState({ fee_board:id, selectedFeeBoard:name, status:status });
  }
  ...
  rendersiblings = () =>{
    ...
    if(siblingCount > 0){
      for(let i = 0 ; i < siblingCount ; i++) {
        lsiblingholders.push({sibling_first_name:'', sibling_middle_name:'', sibling_last_name:'', sibling_gender:'',sibling_date_of_birth:moment(new Date).format("MM/DD/YYYY"), sibling_standard:'',sibling_id_board:'',sibling_school_name:'',sibling_school_address:'',sibling_institute:""});
      }
      this.setState({individualSiblings:lsiblingholders, siblingCount:lsiblingholders.length});
    }
  }
  ...
  handleUpdate(id, order){
    ...
    formData.append('sibling_id',this.state.sibling_id);
    ...
    formData.append('overall_min_mark',this.state.overall_min_mark);
    formData.append('overall_min_per',this.state.overall_min_per);

    new Service().apiCall('Preadmissions/updateStudent', formData,
    {
      headers: { 
      ...
      if (response.status==200) { 
        ...
        setTimeout(() => {
           ...
           this.props.history.push({pathname: '/admin/preadmission-student-info/'+this.state.student_uid})
        }, 2000)
      } else {
  ...
  handleChangeState = (name,value) => {
    ...
    let numberCheck = ["passed_academic_year", "admission_number", "overall_min_mark", "overall_min_per"];
  ...
  studentProfiling = () => {
    ...
    console.log('profile'+this.state.individualAllData);
    return(
      <div className="w-100">
      ...
      <Grid container spacing={2}>
        ...
        <Grid item xs={12} lg={4} className="py-1">
          {this.renderTextInput("overall_min_mark","Overall Minimum Mark")}
        </Grid>
        <Grid item xs={12} lg={4} className="py-1">
          {this.renderTextInput("overall_min_per","Overall Minimum Percentage")}
        </Grid>
      </Grid>
    ...
    return (
      ...
      {this.state.individualSiblings.map((sibling, idx) => (
        <>
          ...
          <FormControlLabel value="end" control={
              <Radio color="primary" checked={sibling.sibling_institute==="Yes"}
                    onChange={() => {this.handleChangeInstitute(idx,"Yes")}} />} label="Yes" 
                    disabled={this.state.editStudent?false:true}/>
          <FormControlLabel value="end" control={<Radio color="primary" checked={sibling.sibling_institute==="No"}
          ...
          {sibling.sibling_institute === "Yes" && <div>
          <Grid container spacing={2}>
          ...
          /> 
            ...
            {sibling.sibling_institute == "No" && <div>
            <Grid container spacing={2}>
          <Grid item xs={12} lg={4} className="py-1">
              <FormControl fullWidth>
                <TextField 
          ...
          <FormControl fullWidth>
            ...
            format="MM/dd/yyyy"
            disabled={this.state.editStudent?false:true}
            value={sibling.sibling_date_of_birth}
            onChange={(val) => this.handleSiblingData(idx,"sibling_date_of_birth",val)}   
            KeyboardButtonProps={{
              'aria-label': 'change date',
            }}
          />
          ...
          <Grid item xs={12} lg={3} className="py-1">
            <FormControl fullWidth>
                  <TextField                        
                      ...
                      className="m-2"
                      disabled={this.state.editStudent?false:true}
                      value={sibling.sibling_id_board}
                      onChange={(event) => this.handleSiblingData(idx,"sibling_id_board",event.target.value)}>
          ...
          <Grid item xs={12} lg={4} className="py-1">
                  <FormControl fullWidth>
                    ...
                    id="document-type"   
                    value={sibling.sibling_school_name}
                    label="School name" 
                    type="search" 
                    disabled={this.state.editStudent?false:true}
                    onChange={(event) => this.handleSiblingData(idx,"sibling_school_name",event.target.value)}
                    inputRef={this.textInput} 
                    variant="outlined" />
                    </FormControl>
          </Grid>
      ...
      <Grid container spacing={2} className="mt-2">
         ...
         { this.state.editStudent  && <div> <Button variant="outlined" className="warningBtnOutline mx-2"  style={{color:'#000000',border:'1px solid #ffc107'}} onClick={()=>this.setState({editStudent:false})}>Cancel</Button>
               ...
               <Button variant="outlined" size="sm" className="successBtnOutline mx-2" style={{color:'#4caf50',border:'1px solid #4caf50'}}  onClick={()=>this.handleUpdate(this.state.student_id,'3')}>
                          Submit
               </Button>
               </div>}
             </Grid>
  ...
  feeConfiguration = () => {
    //console.log('fee_category'+this.state.fee_id);
    return(
      
      <div className="w-100">
        {/* {this.state.individualFees.map((element, index) => 
        <> */}
  ...
  getStudentInfo() { 
    ...
    new Service().apiCall('Preadmissions/getDataWithId', postData).then(response => {
      ...
      if (response.status==200 && response.data!='') {
        ...
        //console.log(response.data.feeconfig); 
  ...
  handlePreadmissionStatus = () => {

    const UID = window.location.href.substring(window.location.href.lastIndexOf('/') + 1);
    const postData = {
      preadmission_status : this.state.preadmission_status,
    }

    // new Service().apiCall('Preadmissions/updateStudent', postData).then(response => { 
    //   //console.log('res'+JSON.stringify(response));
    //   if (response.status==200) { 
    //     this.setState({
    //       basicNotify: (
    ...
    <Fragment>
      ...
      <Grid container spacing={4} className="sliderDiv" justify="center">
        <Grid item xs={12} md={6} lg={6}>
          <Card className="card-box m-3 p-43">
            <CardContent>
              ...
              <div>
                ...
                <div className="font-size-sm rounded-sm">
                  <div className="d-flex justify-content-between mb-3">
                    <span className="font-weight-bold">Born Between:</span>
                    <span className="text-black-50">{moment(this.state.born_start_date).format("MMMM d, YYYY")+ " and "+moment(this.state.born_end_date).format("MMMM d, YYYY")}</span>
                  </div>
                  <div className="d-flex justify-content-between mb-3">
                    <span className="font-weight-bold">Overall Minimum Mark:</span>
                    <span className="text-black-50">{this.state.overall_minmarks}</span>
                  </div>
                  <div className="d-flex justify-content-between mb-3">
                    <span className="font-weight-bold">Overall Minimum Percentage:</span>
                    <span className="text-black-50">{this.state.overall_minpercentage}%</span>
                  </div>
                </div>
              </div>

              <div conatiner spacing={2}>
                <div item xs={12} md={6} lg={6}>
                  {((moment(this.state.dateOfBirth).format("MMMM d, YYYY") >= moment(this.state.born_start_date).format("MMMM d, YYYY") && moment(this.state.dateOfBirth).format("MMMM d, YYYY") <= moment(this.state.born_end_date).format("MMMM d, YYYY"))) ? 
                    <>
                      <Button color="primary" variant='contained'>Elligible</Button>
                    </>
                    :
                    <>
                      <Button color="primary" variant='contained'>Not Elligible</Button>
                    </>
                  }
                </div>
                <div item xs={6} md={6} lg={4}>
                  <RadioGroup row aria-label="position" name="preadmission_status" defaultValue="top" onChange = {(event) => this.setState({preadmission_status:event.target.value})}>        
                    <FormControlLabel value="accepted" control={<Radio color="primary" checked={this.state.preadmission_status == "accepted" ? true:false} />} label="Accept" />
                    <FormControlLabel value="rejected" control={<Radio color="primary" checked={this.state.preadmission_status == "rejected" ? true:false} />} label="Reject" />
                  </RadioGroup>
                  <Button color="primary" size="medium" variant='contained' onClick={()=>this.handlePreadmissionStatus}>Not Elligible</Button>
                </div>                
              </div>
